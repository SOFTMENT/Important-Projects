...
public class SeeAllChallengesAdapter  extends RecyclerView.Adapter<SeeAllChallengesAdapter.ViewHolder> {
    ...
    public SeeAllChallengesAdapter(Context context,ArrayList<ChallnageModel> challnageModels) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.see_all_challenges_layout,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull SeeAllChallengesAdapter.ViewHolder holder, int position) {
        holder.setIsRecyclable(false);


        ChallnageModel challnageModel = challnageModels.get(position);
        Glide.with(context).load(challnageModel.profileImage).placeholder(R.drawable.placeholder).into(holder.profilePicture);
        holder.name.setText(challnageModel.getName());
        Glide.with(context).load(challnageModel.vehicleImage).placeholder(R.drawable.placeholder1).into(holder.vehicleImage);
        holder.email.setText(challnageModel.getEmail());
        holder.more.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showPopupMenu(holder.more,challnageModel.cid, challnageModel.name);
            }
        });
        ...
        FirebaseFirestore.getInstance().collection("Challenges").document(challnageModel.cid).collection("Accepted").document(UserModel.data.uid).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            ...
            public void onComplete(@NonNull @NotNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        holder.acceptChallnage.setEnabled(false);
                        holder.acceptChallnage.setText("Accepted");
                    } else {
                        holder.acceptChallnage.setEnabled(true);
                        holder.acceptChallnage.setText("Accept Challenge");
                    }
                }
            }
        });
        ...
        holder.acceptChallnage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private void showPopupMenu(ImageView more,String cid, String name){
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            ...
            if (item.getItemId() == R.id.report) {
                ...
                View view1 = ((SeeAllChallengeActivity)context).getLayoutInflater().inflate(R.layout.report_reason_layout,null);
