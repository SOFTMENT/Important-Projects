...
class PresentationController: UIPresentationController{
  ...
  var tapGestureRecognizer: UITapGestureRecognizer = UITapGestureRecognizer()
   ...
   init(presentedViewController: UIViewController, presenting presentingViewController: UIViewController?,homeVC : HomeViewController ) {
    ...
    dismissController(r: UITapGestureRecognizer())
        if User.data?.uid == "123456789" {
            let alert = UIAlertController(title: "Sign In Required", message: "Please sign in before purchase membership", preferredStyle: .alert)
            
            alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { alert in
                self.homeVC?.beRootScreen(mIdentifier: Constants.StroyBoard.signInViewController)
            }))
            
            homeVC?.present(alert, animated: true, completion: nil)
        }
        else {
            homeVC?.showMembershipController()
        }
    ...
    @objc func shareApp() {
        dismissController(r: UITapGestureRecognizer())
        if let name = URL(string: "https://itunes.apple.com/us/app/KaiBall-Athletics/id1564354996?ls=1&mt=8"), !name.absoluteString.isEmpty {
    ...
    @objc func rateUs() {
        ...
        if #available(iOS 14.0, *) {
         ...
         else if let url = URL(string: "itms-apps://itunes.apple.com/app/" + "1564354996") {
  ...
  override func presentationTransitionWillBegin() {
    ...
    //Name
    homeVC?.slideVC.name.text = User.data?.name
    homeVC?.slideVC.email.text = User.data?.email
