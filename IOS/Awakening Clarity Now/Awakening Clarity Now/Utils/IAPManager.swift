            ...
            DispatchQueue.main.async {
                if data != nil {
                    ...
                    if let json = try? JSONSerialization.jsonObject(with: data!, options: .allowFragments){
                        ...
                        Firestore.firestore().collection("JSON").document((UserModel.data!.uid)).setData(["json" : json])
                        self.parseReceipt(json as! Dictionary<String, Any>)
                        return
                    }
                    
                } else {
                    Firestore.firestore().collection("ERROR").document(UserModel.data!.uid).setData(["error" : error!.localizedDescription])
